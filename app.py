...
if menu == "⚡ Dashboard":
    ...
    if not df_pres_alert.empty:
        ...
        df_pres_alert['Data_Scadenza'] = pd.to_datetime(df_pres_alert['Data_Scadenza'], errors='coerce').dt.date
        
        # Filtro: Non restituiti E Scaduti (data < oggi)
        scaduti = df_pres_alert[
            (df_pres_alert['Restituito'] != True) & 
            (df_pres_alert['Data_Scadenza'] < date.today()) &
        ...
        if not scaduti.empty:
            ...
            for i, row in scaduti.iterrows():
                st.markdown(f"🔴 **{row['Oggetto']}** - {row['Paziente']} (Scaduto il {row['Data_Scadenza'].strftime('%d/%m')})")
            st.divider()
    # ------------------------------
