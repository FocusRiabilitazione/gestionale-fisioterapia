...
<style>
    ...
    .border-purple { border-left: 4px solid #9f7aea !important; } /* Nuovo colore per Preventivi */
...
with st.sidebar:
    ...
    st.divider(); st.caption("App v65 - Final + Quote Alert")
...
if menu == "⚡ Dashboard":
    ...
    if not df.empty:
        ...
        totali = len(df)
        cnt_prev = len(df_prev) # Conto preventivi
        ...
        # --- 1. KPI CARDS (ORA 5 COLONNE) ---
        col1, col2, col3, col4, col5 = st.columns(5) # Modificato a 5
        ...
        if da_richiamare.empty and visite_passate.empty and visite_imminenti.empty and prev_scaduti.empty: st.success("Tutto tranquillo! Nessun avviso.")
