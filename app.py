    ...
    if not df.empty:
        ...
        if not da_richiamare.empty:
            ...
            for i, row in da_richiamare.iterrows():
                ...
                with c_btn1:
                    if st.button("✅ Rientrato", key=f"rk_{row['id']}", use_container_width=True, type="primary"): update_generic("Pazienti", row['id'], {"Disdetto": False, "Data_Disdetta": None}); st.rerun()
                with c_btn2:
                    if st.button("📅 Rimandare", key=f"pk_{row['id']}", use_container_width=True, type="secondary"): update_generic("Pazienti", row['id'], {"Data_Disdetta": pd.Timestamp.now() + timedelta(days=7)}); st.rerun()
        ...
        if not visite_passate.empty:
            ...
            for i, row in visite_passate.iterrows():
                ...
                with c_btn1:
                    if st.button("✅ Rientrato", key=f"vk_{row['id']}", use_container_width=True, type="primary"): update_generic("Pazienti", row['id'], {"Visita_Esterna": False, "Data_Visita": None}); st.rerun()

        if da_richiamare.empty and visite_passate.empty: st.success("Tutto tranquillo! Nessun avviso.")
    ...
    with tab1:
        with st.expander("📋 Listino Prezzi Attuale", expanded=False):
            if not df_srv.empty and 'Area' in df_srv.columns:
                for area in df_srv['Area'].unique():
        ...
        with st.container(border=True):
            ...
            selected_services_default = []; default_descrizione = "" 
            
            if not df_std.empty:
                opt_std = ["-- Seleziona --"] + sorted(list(df_std['Nome'].unique()))
                scelta_std = st.selectbox("Pacchetto Standard:", opt_std)
                if scelta_std != "-- Seleziona --":
                    ...
                    if row_std.get('Contenuto'):
                        for p in row_std['Contenuto'].split(','):
                            if ' x' in p: srv, qty = p.split(' x'); selected_services_default.append(srv.strip()); st.session_state[f"qty_preload_{srv.strip()}"] = int(qty)

            nomi_pazienti = ["Seleziona..."] + sorted([f"{r['Cognome']} {r['Nome']}" for _, r in df_paz.iterrows()]) if not df_paz.empty else []
            col_paz, col_serv = st.columns([1, 2])
            with col_paz: paziente_scelto = st.selectbox("Intestato a:", nomi_pazienti)
            
            listino_dict = {str(r['Servizio']): float(r.get('Prezzo', 0) or 0) for _, r in df_srv.iterrows()}
            with col_serv: servizi_scelti = st.multiselect("Trattamenti:", sorted(list(listino_dict.keys())), default=[s for s in selected_services_default if s in listino_dict])

            note_preventivo = st.text_area("Descrizione Percorso:", value=default_descrizione, height=100)
            ...
            if servizi_scelti:
                ...
                for s in servizi_scelti:
                    ...
                    with c3: costo = listino_dict[s] * qty; st.write(f"**{costo} €**"); totale += costo
                    righe_preventivo.append({"nome": s, "qty": qty, "tot": costo})
                
                st.divider(); st.markdown(f"### TOTALE: {totale} €")
            ...
            if st.session_state.get('show_html_preview'):
                ...
                html_code = generate_html_preventivo(paziente_scelto, date.today().strftime("%d/%m/%Y"), note_preventivo, righe_preventivo, totale, LOGO_B64, auto_print=False)
    ...
    with tab2:
        ...
        if not df_prev.empty:
            for i, row in df_prev.iterrows():
                rec_id = row['id']; paz = row.get('Paziente', 'Sconosciuto'); tot = row.get('Totale', 0)
                dett = str(row.get('Dettagli', '')); note_saved = str(row.get('Note', '')); data_c = row.get('Data_Creazione', '')

                with st.container(border=True):
                    c1, c2, c3 = st.columns([3, 1, 1])
                    with c1: st.markdown(f"**{paz}** ({data_c}) - **{tot} €**")
                    with c2:
                        righe_pdf = []
                        if dett:
                    ...
                    with c3:
                        if st.button("🗑️ Elimina", key=f"del_{rec_id}", use_container_width=True): delete_generic("Preventivi_Salvati", rec_id); st.rerun()
