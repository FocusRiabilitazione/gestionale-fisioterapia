...
def save_preventivo_temp(paziente, dettagli_str, totale):
    ...
    record = {"Paziente": paziente, "Dettagli": dettagli_str, "Totale": totale, "Data_Creazione": str(date.today())}
    get_data.clear()
    table.create(record, typecast=True)
        ...
        with c_right:
            ...
            if all_areas:
                ...
                chart = alt.Chart(counts).mark_bar(cornerRadius=5).encode(
                    ...
                    grid=False, labelColor='white', titleColor='white'
        ...
        if not active_loans.empty:
            edited_loans = st.data_editor(
                ...
                column_config={
                    ...
                    "Restituito": st.column_config.CheckboxColumn("Rientrato?", help="Spunta se restituito"),
                    "id": None
                }, hide_index=True, use_container_width=True
            )
