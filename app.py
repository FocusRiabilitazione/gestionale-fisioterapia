...
# Fallback: Se non trova i secrets, chiede input manuale
if not API_KEY or not BASE_ID:
    with st.sidebar:
        with st.expander("⚙️ Configurazione API", expanded=True):
            st.warning("⚠️ Chiavi non trovate nei Secrets.")
            API_KEY = st.text_input("Airtable API Key (pat...)", type="password", key="user_api_key")
            BASE_ID = st.text_input("Base ID (app...)", key="user_base_id")
            
            if not API_KEY or not BASE_ID:
                st.info("Inserisci le chiavi per continuare.")
                st.stop()
...
def get_data(table_name):
    try:
        # --- MODIFICA CRITICA PER ERRORE 429 ---
        # Introduce un ritardo per non sovraccaricare le API
        time.sleep(0.4) 
        # ---------------------------------------
        ...
        # Evita il crash totale, ma mostra l'errore se necessario
...
with st.sidebar:
    ...
    st.divider(); st.caption("App v105 - Fix 429")
                        ...
                        if content:
                            for p in content.split(','):
                                ...
                                if ' x' in p:
                                    ...
                                    # MODIFICA: rsplit invece di split per evitare errori se il nome contiene x
                                    parts = p.rsplit(' x', 1)
                                    if len(parts) == 2:
...
# =========================================================
    ...
    INVENTARIO = {
        "Strumenti Mano": [
            "Flex-Bar Gialla1 5L", "Flex-Bar Gialla2 5L", "Flex-Bar Gialla3 5L",
            "Flex-Bar Verde1 10L", "Flex-Bar Verde2 10L", "Flex-Bar Verde3 10L",
            "Flex-Bar Blu1 15L", "Flex-Bar Blu2 15L", "Flex-Bar Blu3 15L",
            "Flex-Bar Rossa1 25L", "Flex-Bar Rossa2 25L", "Flex-Bar Rossa3 25L", 
            "Grip 1", "Grip 2", "Grip 3", "Grip 4", "Grip 5", "Grip 6",
            "Grip 7", "Grip 8", "Grip 9", "Grip 10", "Grip 11", "Grip 12",
            "Mini Tavola Verde Menta", "Mini Tavola Gialla", "Mini Tavola Viola",
            "Mirror Box Blu-Azzurra", "Mirror Box Gialla-Nera", "Mirror Box Grigio-Menta",
            "Palla 1", "Palla 2", "Palla 3", "Palla 4", "Palla 5", "Palla 6",
            "Palla 7", "Palla 8", "Palla 9", "Palla 10", "Palla 11", "Palla 12", "Palla 13", "Palla 14",
            "Kettlebell 1 4 Kg", "Kettlebell 2 4 Kg", "Kettlebell 3 4 Kg",
            "Kettlebell 1 8 Kg", "Kettlebell 2 8 Kg", "Kettlebell 3 8 Kg",
            "Peso 0,5 Kg", "Peso 1 Kg", "Peso 1,5 Kg", "Peso 2 Kg",
        ],
        "Elettrostimolatore": [
            "Globus Arancione 1", "Globus Arancione 2", "Globus Giallo",
            "I-Tech 1", "I-Tech 2", "I-Tech 3", "I-Tech 4", "I-Tech 5", "I-Tech 6",
            "Compex 1", "Compex 2", "Compex 3", "Chattanooga",
        ],
        "Magnetoterapia": [
            "Magnetoterapia 1", "Magnetoterapia 2", "Magnetoterapia 3",
        ]
    }
